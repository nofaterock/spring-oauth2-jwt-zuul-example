...
public class Oauth2ResourceServerTests {
	...
	public void callHello() throws Exception {
		// curl http://localhost:8082/resource-server-1/hello
	...
	public void callApiMe() throws Exception {
		// curl http://localhost:8082/resource-server-1/me
		this.mockMvc.perform(
			get("/me"))
			.andDo(print())
			.andExpect(status().isUnauthorized());

		// curl -H "authorization: bearer {accessToken}" http://localhost:8082/resource-server-1/me
		this.mockMvc.perform(
			get("/me")
	...
	public void callApiUser() throws Exception {
		// curl http://localhost:8082/resource-server-1/user
		this.mockMvc.perform(
			get("/user"))
			.andDo(print())
			.andExpect(status().isUnauthorized());

		// curl -H "authorization: bearer {accessToken}" http://localhost:8082/resource-server-1/user
		this.mockMvc.perform(
			get("/user")
				.header("authorization", "bearer " + USER_ACCESS_TOKEN))
			.andDo(print())
	...
	public void callApiMaster() throws Exception {
		// curl http://localhost:8082/resource-server-1/master
		this.mockMvc.perform(
			get("/master"))
			.andDo(print())
			.andExpect(status().isUnauthorized());

		// curl -H "authorization: bearer {accessToken}" http://localhost:8082/resource-server-1/master
		this.mockMvc.perform(
			get("/master")
				.header("authorization", "bearer " + USER_ACCESS_TOKEN))
			.andDo(print())
	...
	public void callApiClient() throws Exception {
		// curl http://localhost:8082/resource-server-1/client
		this.mockMvc.perform(
			get("/client"))
			.andDo(print())
			.andExpect(status().isUnauthorized());

		// curl -H "authorization: bearer {accessToken}" http://localhost:8082/resource-server-1/client
		this.mockMvc.perform(
			get("/client")
				.header("authorization", "bearer " + USER_ACCESS_TOKEN))
			.andDo(print())
